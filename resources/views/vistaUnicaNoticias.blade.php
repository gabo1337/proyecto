<!DOCTYPE html>
<!-- saved from url=(0049)https://blogzine.webestica.com/post-single-2.html -->
...
<!-- =======================
Inner intro START -->
<section class="bg-dark-overlay-4" style="background-image:url({{ asset('storage/'.$noticia->image) }}); background-position: center left; background-size: cover;">
	<div class="container">
		<div class="row">
			<div class="col-lg-8 py-md-5 my-lg-5">
        <a class="badge bg-warning mb-2"><i class="fas fa-circle me-2 small fw-bold"></i>Sistemas</a>
            ...
            <div class="nav-link">
              <div class="d-flex align-items-center text-white position-relative">
                <div class="avatar avatar-sm">
                  <img class="avatar-img rounded-circle" src="{{asset('storage/'.$noticia->user->profile[0]->profilephoto)}}" alt="avatar">
                </div>
                <span class="ms-3">by <a href="{{ url('users/'. $noticia->user->id) }}" class="stretched-link text-reset btn-link">{{$noticia->user->name}}</a></span>
              </div>
            </div>
          </li>
          <li class="nav-item">{{$noticia->created_at->toFormattedDateString()}}</li>
          </ul>
...
<section>
	<div class="container position-relative" data-sticky-container="">
		<div class="row">
      	...
      	<p><span class="dropcap"></span>{{$noticia->content}}</p>
